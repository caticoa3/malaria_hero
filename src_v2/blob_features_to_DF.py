...
def gen_bn_features(image_dir='../datasets/cell_images/',
    ...
    for i, file in enumerate(Path(image_dir).glob('**/*.png')):
        if (saved_feat_data.is_file() and (i == 0) and training):
            # file exists
            print('Found', blob_features_file, ', which contains features '
                  'extracted from prior images.\nWe will add to those data.')
            feat_df = pd.read_csv(blob_features_file, index_col=0)
        ...
        if i % 40 == 0:
            ...
            feat_df.to_csv(blob_features_file)
    ...
    if min_samples > 0:
        ...
        print('Saved to filtered_', blob_features_file)
        bn_feat_for_ml.to_csv('filtered_' + blob_features_file)
